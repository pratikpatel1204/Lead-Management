...
@section('title', config('app.name') . ' || Why Choose Us')
                ...
                <div class="card-body">
                    <form id="whyChooseUsForm" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="title" class="form-control" value="{{ $choose_us->title ?? '' }}" placeholder="Enter Title" required>
                            <span class="text-danger error-title"></span>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Short Description</label>
                            <textarea name="short_description" class="form-control" rows="3" placeholder="Enter Short Description">{{ $choose_us->short_description ?? '' }}</textarea>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="list_one" class="form-control" value="{{ $choose_us->list_one ?? '' }}" placeholder="Enter list item">
                                <span class="text-danger error-list_one"></span>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">List Item 2</label>
                                <input type="text" name="list_two" class="form-control" value="{{ $choose_us->list_two ?? '' }}" placeholder="Enter list item">
                                <span class="text-danger error-list_two"></span>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">List Item 3</label>
                                <input type="text" name="list_three" class="form-control" value="{{ $choose_us->list_three ?? '' }}" placeholder="Enter list item">
                                <span class="text-danger error-list_three"></span>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">List Item 4</label>
                                <input type="text" name="list_four" class="form-control" value="{{ $choose_us->list_four ?? '' }}" placeholder="Enter list item">
                        ...
                        <div class="mb-3">
                            ...
                            @if (!empty($choose_us->image))
                                <div class="mt-2">
                                    <img src="{{ asset($choose_us->image) }}" alt="Current Image" width="120" class="rounded">
                                </div>
                            @endif
                        ...
                        <div class="text-end">
                            <button type="submit" id="saveBtn" class="btn btn-primary">
                                <span class="btn-text">{{ $choose_us ? 'Update' : 'Save' }}</span>
...
$("#whyChooseUsForm").on('submit', function(e) {
    ...
    $.ajax({
        url: "{{ route('admin.why.choose.us.update') }}",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
